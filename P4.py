...
# ---
# 
# * Estudiante: **José Gabriel Solano Ugarte**
# * Carné: **B87668**
# * Grupo: **1**
# 
# ---
...
# Tomando como referencia el análisis de 4 bits y la base de pasos
...
def modulador_16QAM(bits, fc, mpp):
    ...
    :param fc: Frecuencia de la portadora en Hz
    :param mpp: Cantidad de muestras por periodo de onda portadora
    :return: Un vector con la señal modulada como senal_Tx
    :return: Un valor con la potencia promedio [W] (P_senal_Tx)
    :return: La  variable portadora1 define la onda portadora coseno c1(t)
    :return: La variable portadora2 define la onda portadora seno c2(t)
    '''
    # 1. Parámetros de la 'señal' de información (bits)
    N = len(bits)  # Cantidad de bits
    ...
    # Se definen cada uno de los casos para la portadora
    # El primero sería para el caso con coseno y el segundo caso con seno.
    ...
    # 4. Asignación de las formas de onda según los bits (16-QAM)
    # Se procede a establece el contador de muestreos j
    # Todo valor se almacena entre j y j+1
    ...
    for i in range(0, N, 4):
        ...
        Se consideran los casos para cada uno de los bits y la ubicación 
        que tienen estos dentro del mapa de la constelación'''
        
        senal_Tx[j*mpp: (j+1)*mpp] =             (-1)**(1+bits[i]) * 3**(1-bits[i+1]) * portadoraI             + (-1)**(bits[i+2]) * 3**(1-bits[i+3]) * portadoraQ
        j += 1
        
    # 5. Calcular la potencia promedio de la señal modulada
    P_senal_Tx = 1 / (N*Tc) * np.trapz(pow(senal_Tx, 2), t_simulacion)

    return senal_Tx, P_senal_Tx, portadoraI, portadoraQ


# In[32]:


# Para complementar la modulación se debe tener el demodulador

def demodulador_16QAM(senal_Rx, portadora1, portadora2, mpp):
    '''Un método que simula un bloque demodulador
    de señales, bajo un esquema 16-QAM.
    :param senal_Rx: La señal recibida del canal
    :param portadora: La onda portadora c(t) (general)
    :param mpp: Número de muestras por periodo
    :return: Los bits de la señal demodulada
    '''
    ...
    #Pseudo-Energía por período en la portadora
    j = 0

    # Demodulación
    for i in range(N):
        #Para esta función se iguala la cantidad de iteraciones
        # Con la cantidad de información que se puede recuperar
        if j+4 > N:  
            break   
...
# Simulación de la modulación 

fc = 5000  # frecuencia de la portadora
mpp = 20   # muestras por periodo de la portadora
SNR = 16   # relación señal-a-ruido del canal
...
senal_Tx, Pm, portadoraI, portadoraQ = modulador_16QAM(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador_16QAM(senal_Rx, portadoraI, portadoraQ, mpp)
...
# Se dan los tiempos de graficación
...
A = [-3, -1, 1, 1]             # Se tiene dos posibles valores para la amplitud
N = 8                          # Hay 8 posibilidades de la onda, cuatro en cada caso
B_t = np.empty((N, len(t)))    # x(t) con T puntos en N funciones del tiempo
C_t = np.empty((N, len(t)))
D_t = np.empty((N, len(t)))
E_t = np.empty((N, len(t)))

# Se aplican los valores de amplitud para la señal (-3, -1, 1 y 3)
for i in A:
...
P = [np.mean(B_t[:,i]) for i in range(len(t))]
Rea, = plt.plot(t, P, lw=6, color = 'darkgreen', label='Promedio realizaciones')

# Graficar el resultado teórico del valor esperado

E = np.mean(senal_Tx)*t
Teo, = plt.plot(t, E, '-.', lw=3, color = 'pink', label='Valor teorico')
...
for n in range(N):
    for i, tau in enumerate(desplazamiento):
        corr[n, i] = np.correlate(B_t[n,:], np.roll(B_t[n,:], tau))/T
...
plt.plot(taus, Rxx, '-.', lw=4, color = 'seagreen', label='Correlación teórica')
